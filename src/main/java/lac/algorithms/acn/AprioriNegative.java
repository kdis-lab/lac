...
public class AprioriNegative {
    ...
    private ArrayList<Rule> negativeRules;

    /**
     * Default constructor
     */
    public AprioriNegative(Dataset dataset, Config config) {
     ...
     * 
     * @return rules whose support, confidence and all-confidence is greater than a
     *         user-specified threshold
     */
    public ArrayList<Rule> run() {
        ArrayList<Rule> rules = new ArrayList<Rule>();
        negativeRules = new ArrayList<Rule>();
        ...
        do {
            ...
            for (Rule candidate : candidatesK) {
                if (candidate.getSupportRule() >= this.minSupRelative) {
                    ...
                    // Generate a negative rule per each item contained in the antecedent
                    for (int m = 0; m < candidate.getAntecedent().size(); m++) {
                        Rule negatedRule = (Rule) candidate.clone();
