...
public class ConfigTest extends TestSuite {
    ...
    public void eachParameterHasADefaultValue() {
        ...
        assertEquals(0.1, config.getMinMerit(), 0.0);
    }

    @Test
    public void eachParameterAcceptsBothIntegerAndDouble() {
        config.setMinMerit(0.1);
        assertEquals(0.1, config.getMinMerit(), 0.0);

        config.setMinMerit(new Integer(0));
        assertEquals(0, config.getMinMerit(), 0.0);
    ...
    public void configAsStringShowsAllFields() {
        assertEquals("minConf=0.5 minMerit=0.1", config.toString());
    }
}
