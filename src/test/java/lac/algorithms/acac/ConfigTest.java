...
public class ConfigTest extends TestSuite {
    ...
    public void eachParameterHasADefaultValue() {
        assertEquals(0.02, config.getMinSup(), 0.0);
        assertEquals(1.0, config.getMinConf(), 0.0);
        assertEquals(0.5, config.getMinAllConf(), 0.0);
    }
    ...
    public void eachParameterAcceptsBothIntegerAndDouble() {
        ...
        config.setMinAllConf(0.1);
        assertEquals(0.1, config.getMinAllConf(), 0.0);

        config.setMinAllConf(new Integer(0));
        assertEquals(0, config.getMinAllConf(), 0.0);
    }

    @Test
    public void configAsStringShowsAllFields() {
        assertEquals("minSup=0.02 minConf=1.0 minAllConf=0.5", config.toString());
    }
}
