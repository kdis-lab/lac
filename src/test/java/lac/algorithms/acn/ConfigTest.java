...
public class ConfigTest extends TestSuite {
    ...
    public void eachParameterHasADefaultValue() {
        ...
        assertEquals(0.55, config.getMinAcc(), 0.0);
        assertEquals(0.2, config.getMinCorr(), 0.0);
    }
    ...
    public void eachParameterAcceptsBothIntegerAndDouble() {
        ...
        config.setMinAcc(0.1);
        assertEquals(0.1, config.getMinAcc(), 0.0);

        config.setMinAcc(new Integer(0));
        assertEquals(0, config.getMinAcc(), 0.0);

        config.setMinCorr(0.1);
        assertEquals(0.1, config.getMinCorr(), 0.0);

        config.setMinCorr(new Integer(0));
        assertEquals(0, config.getMinCorr(), 0.0);
    }

    @Test
    public void configAsStringShowsAllFields() {
        assertEquals("minSup=0.01 minConf=0.5 minCorr=0.2 minAcc=0.55", config.toString());
    }
}
