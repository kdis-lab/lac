...
public class ConfigTest extends TestSuite {
    ...
    public void eachParameterHasADefaultValue() {
        ...
        assertEquals(4, config.getDelta(), 0.0);
    }
    ...
    public void eachParameterAcceptsBothIntegerAndDouble() {
        ...
        config.setDelta(1);
        assertEquals(new Integer(1), config.getDelta());

        config.setDelta(new Integer(0));
        assertEquals(new Integer(0), config.getDelta());
    }

    @Test
    public void configAsStringShowsAllFields() {
        assertEquals("minSup=0.01 minConf=0.5 delta=4", config.toString());
    }
}
